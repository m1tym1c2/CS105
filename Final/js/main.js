...
let isDownloading = false; // Flag to ensure single execution

// Function to download GLB and JSON files
function download(startIndex = 1) {
    ...
    document.querySelectorAll('a.download-link').forEach(link => link.remove());

    // Track the number of objects processed
    ...
    meshObject.forEach((obj, index) => {
        ...
        // Create a new scene for each object
        const exportScene = new THREE.Scene();
        exportScene.add(clonedObj);

        // Prepare the file name with the specified start index
        const fileName = `Object_${startIndex + index}.glb`;

        // Use GLTFExporter to convert the scene to a GLB file
        ...
        exporter.parse(
            ...
            function (result) {
                if (result instanceof ArrayBuffer) {
                    ...
                    convertJSONToGLB(result, fileName);
                }

                // Get object details
                const boundingBox = new THREE.Box3().setFromObject(clonedObj);
                const size = boundingBox.getSize(new THREE.Vector3());
                const position = clonedObj.position;
                const rotation = clonedObj.rotation;

                // Add details to JSON data
                jsonData.data.push({
                    name: `Object_${startIndex + index}`,
                    size: {
                        horizontal_size: size.x,
                        vertical_size: size.y
                    },
                    position: {
                        horizontal_position: position.x,
                ...
                // Save JSON data after processing all objects
                if (processedObjects === meshObject.length) {
                    saveJSONData(jsonData, startIndex);
                }
            },
            { binary: true }
        );
...
// Function to save ArrayBuffer data
function saveArrayBuffer(buffer, fileName) {
    const blob = new Blob([buffer], { type: 'application/octet-stream' });
    save(blob, fileName);
}

// Function to save Blob data
...
// Function to convert JSON result to ArrayBuffer and save
function convertJSONToGLB(json, fileName) {
    const string = JSON.stringify(json);
    const blob = new Blob([string], { type: 'application/json' });
    const reader = new FileReader();
    reader.readAsArrayBuffer(blob);
    reader.onloadend = function() {
        saveArrayBuffer(reader.result, fileName);
    };
}

function saveJSONData(data, startIndex) {
    const jsonString = JSON.stringify(data, null, 2);
    const blob = new Blob([jsonString], { type: 'application/json' });
    const fileName = `objects_data_${startIndex}.json`;
    const link = document.createElement('a');
    link.href = URL.createObjectURL(blob);
    link.download = fileName;
    link.className = 'download-link';
...
function promptAndDownload() {
    const startIndex = parseInt(prompt("Enter the starting index for object files:", "1"), 10);
