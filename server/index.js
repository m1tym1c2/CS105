...
const updateFromFile = (filePath) => {
    const rawData = fs.readFileSync(filePath);
    const jsonData = JSON.parse(rawData);
  
    if (jsonData.data && Array.isArray(jsonData.data)) {
      jsonData.data.forEach((obj) => {
        const { name, size, position, rotation } = obj;
        items[name] = {
          name: name,
          size: [size.horizontal_size, size.vertical_size],
        };
  
        map.items.push({
          ...items[name],
          gridPosition: [position.horizontal_position, position.vertical_position],
          rotation: rotation,
        });
      });
      ...
      console.error(`Invalid data in file: ${filePath}`);
    }
  };
  
  // Hàm để load tất cả các file JSON trong thư mục 'data'
  const loadAllFiles = () => {
    ...
    fs.readdir(directoryPath, (err, files) => {
      ...
      files.forEach((file) => {
        if (path.extname(file) === '.json') {
          updateFromFile(path.join(directoryPath, file));
        }
      });
    });
  };
  
  // Load tất cả các file khi khởi động
  loadAllFiles();

// Theo dõi thư mục 'data' để cập nhật khi có file mới
